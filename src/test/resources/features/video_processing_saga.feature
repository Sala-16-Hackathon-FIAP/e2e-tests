Feature: Video Processing Saga
  ...
  Scenario: Happy path - upload a video and receive processed frames
    ...
    Then the upload should be created with status "INITIATED"
    ...
    And the video-processor should eventually create a job with status "COMPLETED"
  ...
  Scenario: Failed processing - user is notified of the error
    ...
    Then the video-processor should eventually create a job with status "FAILED"
